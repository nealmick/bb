    ...
    for s in range(len(seasons)):
        ...
        for game in sorted:
            ...
            for i in range(0,playersPerTeam):
                ...
                visitorTeam.pop(b)
            foo+=1
            history,history2 = writeCSV(game,g['spread'],g['home_score'],g['visitor_score'],g['home_id'],g['visitor_id'],homeTeamStats,visitorTeamStats,bestH,bestV,path,season,foo,beforeStreaks,history,history2)


def writeCSV(game,spread, homeScore,visitorScore,homeId,visitorId,homeTeamStats,visitorTeamStats,bestH,bestV,path,season,foo,streaks,history,history2):
    ...
    try:
        hh2 = history2[homeId]
        vh2 = history2[visitorId]
        hh = history[homeId]
        vh = history[visitorId]
    except KeyError:
        history[homeId] = line
        history[visitorId] = line
        history2[homeId] = line
        history2[visitorId] = line
        return history,history2
    history[homeId] = line
    history[visitorId] = line
    history2[homeId] = hh
    history2[visitorId] = vh

    if season == '2020':
        if foo > 750:#sets split of test/train on final season.....
            csv = open('csv/test.csv','a')
            csv.write(line+','+hh+','+vh+','+hh2+','+vh2+'\n')
